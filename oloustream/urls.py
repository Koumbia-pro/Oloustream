"""
URL configuration for oloustream project.

The `urlpatterns` list routes URLs to views. For more information please see:
    https://docs.djangoproject.com/en/5.0/topics/http/urls/
Examples:
Function views
    1. Add an import:  from my_app import views
    2. Add a URL to urlpatterns:  path('', views.home, name='home')
Class-based views
    1. Add an import:  from other_app.views import Home
    2. Add a URL to urlpatterns:  path('', Home.as_view(), name='home')
Including another URLconf
    1. Import the include() function: from django.urls import include, path
    2. Add a URL to urlpatterns:  path('blog/', include('blog.urls'))
"""

from django.contrib import admin
from django.urls import path, include
from django.conf import settings
from django.conf.urls.static import static
from django.contrib.auth import views as auth_views

urlpatterns = [
    path('admin/', admin.site.urls),

    # Front / site vitrine
    path('', include('apps.core.urls', namespace='core')),

    # Auth / comptes
    path('accounts/', include('apps.accounts.urls', namespace='accounts')),

      # Mot de passe oublié (sans namespace, noms globaux)
    path('password-reset/', auth_views.PasswordResetView.as_view(
        template_name='user/auth/password_reset.html'
    ), name='password_reset'),
    path('password-reset/done/', auth_views.PasswordResetDoneView.as_view(
        template_name='user/auth/password_reset_done.html'
    ), name='password_reset_done'),
    path('reset/<uidb64>/<token>/', auth_views.PasswordResetConfirmView.as_view(
        template_name='user/auth/password_reset_confirm.html'
    ), name='password_reset_confirm'),
    path('reset/done/', auth_views.PasswordResetCompleteView.as_view(
        template_name='user/auth/password_reset_complete.html'
    ), name='password_reset_complete'),

    # Dashboard admin
    path('dashboard/', include('apps.dashboard.urls', namespace='dashboard')),

    # Studio / équipements / réservations
    path('studio/', include('apps.studio.urls', namespace='studio')),

    # Services, offres, formations, partenaires
    path('services/', include('apps.services_app.urls', namespace='services_app')),

    # Paiements
    path('payments/', include('apps.payments.urls', namespace='payments')),

    # Messagerie / chat
    path('messaging/', include('apps.messaging.urls', namespace='messaging')),

    # Notifications
    path('notifications/', include('apps.notifications.urls', namespace='notifications')),

]

if settings.DEBUG:
    urlpatterns += static(settings.MEDIA_URL, document_root=settings.MEDIA_ROOT)
